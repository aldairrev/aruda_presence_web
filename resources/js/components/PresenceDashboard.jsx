...
export default function PresenceDashboard() {
    ...
    const fetchData = async () => {
        try {
            ...
            if (error.response && error.response.status === 403) {
                setAuthorized(false);
                setClientIp(error.response.data.client_ip || '');
            } else {
    ...
    if (!authorized) {
        return (
            <div className="w-full max-w-[450px] mx-auto flex flex-col gap-6">
                <section className="presence-card flex flex-col p-8 min-h-[250px] relative overflow-hidden glass-card rounded-[24px] text-center justify-center items-center gap-5">
                    <div className="w-16 h-16 rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center text-red-400 shadow-[0_0_20px_rgba(239,68,68,0.15)] mb-2">
                        <i className="fa-solid fa-lock text-2xl"></i>
                    </div>
                    <div>
                        <h2 className="text-xl font-bold text-slate-100">
                            {t('visitor_logs.unauthorized_title')}
                        </h2>
                        <p className="text-xs text-slate-400 mt-2.5 leading-relaxed">
                            {t('visitor_logs.unauthorized_desc')}
                        </p>
                    </div>
                </section>
            </div>
            ...
            <header className="flex flex-col sm:flex-row items-center p-6 sm:px-8 sm:py-6 gap-6 relative glass-card rounded-[24px]">
                ...
                <div className="flex-1 text-center sm:text-left">
                    <div className="flex items-center justify-center sm:justify-start gap-3">
                        ...
                        <span className={`text-[10px] font-semibold tracking-wider uppercase px-2.5 py-1 rounded-full bg-white/5 border border-white/5 status-badge ${isSteamIngame ? 'text-steam-ingame border-steam-ingame/20 bg-steam-ingame/5' :
